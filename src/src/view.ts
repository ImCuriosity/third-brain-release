...
export class ThirdBrainView extends ItemView {
	...
	private buildHeader(root: HTMLElement) {
		...
		activeDocument.addEventListener('click', () => donatePopup.removeClass('is-visible'), { passive: true });
	...
	private async openNativeGraph(folders: string[]): Promise<void> {
		...
		for (const targetMs of [200, 500, 1000, 1800]) {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Obsidian graph view has no public TypeScript API; cast needed to access renderer/getState
	...
	private async renderBridgeResult(
		...
		if (highConf.length > 0) {
			...
			// 그래프 자동 표시
			window.setTimeout(() => this.openNativeGraph([folderAName, folderBName]), 300);
		}
